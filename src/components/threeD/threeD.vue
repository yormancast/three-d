...
export default defineComponent({
  ...
  setup() {
    ...
    const getNavItem = (itemName) => {
      return dataModel.value.filter((el) => {
        return el.name === itemName
      });
    };

    const resetVisibility = () => {
      dataModel.value.forEach((element) => {
        element.visible = false;
      });
    };

    const handleNavClickEvent = (itemClicked) => {
      const currentNavItem = getNavItem(itemClicked)[0];
